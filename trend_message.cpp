...
trend_word::trend_word () : data (0x00)
{
}


trend_word::trend_word (uint32_t n) : data (n)
{
}

trend_word::trend_word (const unsigned char *p)
{
    data = (((uint32_t)p[0]) << 0) + (((uint32_t)p[1]) << 8) + (((uint32_t)p[2]) << 16) +
           (((uint32_t)p[3]) << 24);
}
...
message_data::message_data () : scalar_value (0), vector_value (0)
{
}
    ...
    if (i == md.end ())
        {
            std::cerr << "WARNING: Parameter " << name
                      << " not given by the cfg map, use default value by filling zero" << std::endl;
            return message_data ();
        }
    else
        {
...
std::shared_ptr<base_message>
create_message (std::string message_type, const std::map<std::string, message_data> &data)
{
...
TRENDTRIG::TRENDTRIG (const TRENDTRIG &rhs) : raw_data (rhs.raw_data)
{
}
...
TRENDGPS::TRENDGPS (uint8_t RWb, uint8_t Addr, uint8_t wrd, const std::vector<uint8_t> d) : data (d)
{
    header.zeros = 0;
    header.RWb = RWb;
    header.Addr = Addr;
...
: TRENDGPS (fetch_md ("RWb", md), fetch_md ("Addr", md), fetch_md ("wrd", md), fetch_md ("data", md))
{
}
...
TRENDIntReg::TRENDIntReg (const TRENDIntReg &rhs) : raw_data (rhs.raw_data)
{
}
    ...
    if (raw_data.rw)
        {
            ...
            oss << "write" << std::endl
                ...
                << "dest mac1:" << std::setbase (16)
                << (raw_data.AD2 + ((uint64_t)raw_data.AD3 << 32)) << std::endl
                << "dest ip1:" << ip2str (raw_data.AD4) << std::endl
                << "dest mac2:" << std::setbase (16)
                << (raw_data.AD5 + ((uint64_t)raw_data.AD6 << 32)) << std::endl
...
local_station_t TRENDData::to_local_station_t () const
{
    local_station_header_t lsh{
        0, // length set to zero, and will be updated later
        (std::uint16_t)header.EC, // event nr set to zero
        (std::uint16_t) (
        ...
        0, // channel mask, set to zero
        12, // ADC resolution
        (std::uint16_t)data.size (), // trace length
        0 // version
    };

    return local_station_t (lsh, {}, data);
}
...
TRENDSlc::TRENDSlc ()
{
    raw_data.zero1 = raw_data.zero2 = raw_data.zero3 = raw_data.zero4 = raw_data.zero5 =
    raw_data.zero6 = raw_data.zero7 = 0;
    raw_data.IP = 0;
    raw_data.VPower1 = 0;
    raw_data.VPower2 = 0;
    raw_data.VPower3 = 0;
...
{
    raw_data.zero1 = raw_data.zero2 = raw_data.zero3 = raw_data.zero4 = 0;
...
std::string TRENDRdIntReg::to_string (bool use_hex) const
{
    ...
    oss << "IP:" << ip2str (raw_data.IP) << std::endl
        ...
        << "dest mac1:" << std::setbase (16)
        << (raw_data.dmac1_lsb + ((uint64_t)raw_data.dmac1_msb << 32)) << std::endl
        << "dest ip1:" << ip2str (raw_data.dip1) << std::endl
        << "dest mac2" << std::setbase (16)
        << (raw_data.dmac2_lsb + ((uint64_t)raw_data.dmac2_msb << 32)) << std::endl
...
////////////////////////
TRENDADC::TRENDADC () : data (0x00)
{
}

TRENDADC::TRENDADC (uint8_t addr, uint16_t reg_func)
{
    data = ((uint32_t) (addr & 0x1f) << 11) + (reg_func & 0x7ff);
}

TRENDADC::TRENDADC (const std::map<std::string, message_data> &md)
: TRENDADC (fetch_md ("ADDR", md), fetch_md ("REGFUNC", md))
{
}
...
std::string TRENDADC::to_string (bool use_hex) const
{
    ...
    oss << "ADDR:" << (data >> 11) << std::endl << "REG FUNC:" << (data & 0x7ff);
    return oss.str ();
}
////////////////////////
TRENDACK::TRENDACK ()
...
TRENDACK::TRENDACK (uint32_t IP, uint32_t MsgAck)
{
    raw_data.IP = IP;
...
: TRENDACK (fetch_md ("IP", md), fetch_md ("MsgAck", md))
{
}
...
std::string TRENDACK::to_string (bool use_hex) const
{
    ...
    oss << "IP:" << ip2str (raw_data.IP) << std::endl << "MSGACK:" << raw_data.MsgAck;
    return oss.str ();
}

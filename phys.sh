...
sed -e "s/TrgEn.*/TrgEn\t\tSCALAR\t0b$8/g" temp.cfg > phys$NRUN.cfg

# Clean
tmux kill-window -t "w"
# Execute run
$DAQDIR/run.sh  1236 192.168.1.1${BOARDID} phys$NRUN.cfg $DATADIR/R$NRUN'_b'$BOARDID.data "w" 0

# Log run id
mv phys$NRUN.cfg  $DATADIR/R$NRUN'_b'$BOARDID.cfg

...
struct message_data
{
    ...
    template <typename T> message_data (T v) : scalar_value (static_cast<uint64_t> (v))
    {
    }

    message_data (std::initializer_list<uint64_t> v) : scalar_value (0), vector_value (v)
    {
    }

    template <typename T> message_data (const std::vector<T> &v) : scalar_value (0)
    {
    ...
    template <typename T> std::vector<T> fetch_vector () const
    {
    ...
    template <typename T> T fetch_scalar () const
    {
...
template <typename T> std::deque<trend_word> to_deque (const T &x)
{
...
template <typename T> T from_deque (const std::deque<trend_word> &dq)
{
    static_assert (sizeof (T) % 4 == 0, "Error, the size of type T must be 4x");
    // std::cerr<<sizeof(T);
...
struct TRENDDAQ : public base_message
{
    struct
    {
...
struct TRENDTRIG : public base_message
{
    struct
    {
        unsigned int ST : 1;
...
struct TRENDSlcReq : public base_message
{
    TRENDSlcReq () = default;
...
struct TRENDGPS : public base_message
{
    struct
    {
        unsigned int RWb : 1;
...
struct TRENDIntReg : public base_message
{
    struct
    {
        uint32_t rw;
...
struct TRENDData : public base_message
{
    struct
    {
        uint32_t IP;
    ...
    uint32_t type_code () const;
    std::string to_string (bool use_hex) const;
    local_station_t to_local_station_t () const;
};

struct TRENDSlc : public base_message
{
    struct
    {
        uint32_t IP;
...
struct TRENDRdIntReg : public base_message
{
    struct
    {
        uint32_t IP;
...
struct TRENDADC : public base_message
{
    uint32_t data;
...
struct TRENDACK : public base_message
{
    struct
    {
        uint32_t IP;
    ...
    TRENDACK ();
    TRENDACK (uint32_t IP, uint32_t MsgAck);
